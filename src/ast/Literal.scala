...
case class StringLiteral(str: StringToken) extends Literal {
  ...
  def generateCode(implicit current: List[Int], params: List[String], pathList: List[List[Int]], cus: List[CompilationUnit]) = {
    X86Gen.notImpl // __malloc and stuff X86Mov(X86eax, X86Number(char.getInt)) :: Nil
  }
}
