...
case class ClassDefinition(className: String, parent: Option[RefType], interfaces: List[RefType], modifiers: List[Modifier], fields: List[FieldDeclaration], constructors: List[ConstructorDeclaration], methods: List[MethodDeclaration]) extends TypeDefinition(className) {
  def display: Unit = {
    ...
    for (x <- modifiers) println(s"Modifier: ${Modifier.fromModifier(x)}")
...
case class MethodDeclaration(methodName: String, returnType: Type, modifiers: List[Modifier],
  ...
  def display: Unit = {
    println("*" * 20)
    println("Method declaration")
    println("*" * 20)
    println(s"Method name: $methodName")
    println(s"Return type: ${returnType.typeName}")
    println(s"Number of modifiers: ${modifiers.size}")
    for(x <- modifiers) println(s"Modifier: ${Modifier.fromModifier(x)}")
    println(s"Number of parameters: ${parameters.size}")
    for((x,y) <- parameters) println(s"Parameter type : ${x.typeName}\n          name: $y")
    println(s"Is defined: ${implementation.isDefined}")
    println("*" * 20)
    println()
...
case class FieldDeclaration(fieldName: String, fieldType: Type, modifiers: List[Modifier],
  ...
  def display: Unit = {
    println("*" * 20)
    println("Field declaration")
    println("*" * 20)
    println(s"Field name: $fieldName")
    println(s"Type: ${fieldType.typeName}")
    println(s"Number of modifiers: ${modifiers.size}")
    for(x <- modifiers) println(s"Modifier: ${Modifier.fromModifier(x)}")
    println(s"Field initializer: ${initializer.isDefined}")
    println("*" * 20)
    println()
...
case class ConstructorDeclaration(modifiers: List[Modifier], parameters: List[(Type, String)], implementation: Block) {
  def display: Unit = {
    println("*" * 20)
    println("Constructor declaration")
    println("*" * 20)
    println(s"Number of modifiers: ${modifiers.size}")
    for(x <- modifiers) println(s"Modifier: ${Modifier.fromModifier(x)}")
    println(s"Number of parameters: ${parameters.size}")
